...
center = new THREE.Vector3(0, 0, 0),
    particleDestination = new THREE.Vector3(0, 0, 200); // set some camera attributes

var VIEW_ANGLE = 100,
    ASPECT = WIDTH / HEIGHT,
    NEAR = 0.01,
    FAR = 4000;
var spawnRate = 50; // get the DOM element to attach to
// - assume we've got jQuery to hand

var container = document.querySelector("#container"); // create a WebGL renderer, camera
// and a scene
...
var scene = new THREE.Scene(); // add the camera to the scene

scene.add(camera); // the camera starts at 0,0,0
// so pull it back
...
camera.rotation.z = Math.PI; // start the renderer

renderer.setSize(WIDTH, HEIGHT); // attach the render-supplied DOM element

container.appendChild(renderer.domElement); // create the sphere's material
...
var standardGeometry = new THREE.CylinderGeometry((WIDTH + HEIGHT) * 0.045, // upper radius
(WIDTH + HEIGHT) * 0.045, // lower radius
0.01, // height
4 // segments
);
standardGeometry.applyMatrix(new THREE.Matrix4().makeRotationX(Math.PI / 2)); // create a point light

var pointLight = new THREE.PointLight(0xFFFFFF); // set its position

pointLight.position.x = 0;
pointLight.position.y = 0;
pointLight.position.z = 500; // add to the scene
...
function setupParticle(particle, options) {
  ...
  particle.geometry.dynamic = true; // changes to the vertices

  particle.geometry.verticesNeedUpdate = true; // changes to the normals
  ...
  particle.speed = -1; //	particle.lookAt(camera.position)
...
function recycleParticle(particle) {
  scene.remove(particle); // remove from active list

  var index = activeParticles.indexOf(particle);
  activeParticles.splice(index, 1); // remove from group

  index = groups[particle.groupId].indexOf(particle);
  groups[particle.groupId].splice(index, 1); // return to pool
...
function spawnParticle(options) {
  var particle = _.extend(new THREE.Mesh(standardGeometry, standardMaterial.clone()), {
    age: 0,
    lifespan: 2000,
    birthday: startTime
  }, options);
...
function updateParticle(p, time) {
  ...
  p.scale.x = 1 - p.age;
  p.scale.y = 1 - p.age;
  p.scale.z = 1 - p.age; //p.lookAt(camera.position)
} // ------------------------------------
...
function loadColorMap() {
  ...
  var imgSrc = "http://www.theorigin.net/silkbrush/img/colormabtyhyup.png"; // var imgSrc = "https://scontent-lax3-1.xx.fbcdn.net/hphotos-xtf1/v/t1.0-9/11049578_10101885156149034_3791560155492479655_n.jpg?oh=20f2a8913950448f1a949e06a21554bb&oe=56C22952";
